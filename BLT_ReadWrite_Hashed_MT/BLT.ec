...
type pkey, skey, tag, message, RI, chain.
op keys : (pkey * skey) distr.
op toRI : message * tag -> RI.
op queryBound : int.
op checkRepo : RI option -> chain -> RI -> bool.
op tdistr : int distr.
...
clone import Timestamp.RepoType with type d <- RI,
                           type o <- RI,
                           type r <- int.
...
module BLT_Scheme(Tag_O : Tag_Oracle, Ts_O : TS) = {
  ...
  proc verify(m : message, t : tag, c : chain) : bool = {
    var b  : bool;
    var s' : RI option;
    var tm : int;
    ...
    return b /\ checkRepo s' c (toRI (m, t));  
  }
}.
...
module type BLT_Adv(O : BLT_AdvOracle)  = {
  proc forge(pk : pkey, rs : int) : message * tag * chain 
}.
